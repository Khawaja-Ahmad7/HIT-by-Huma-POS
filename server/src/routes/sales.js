...
router.post('/', [
  ...
  try {
    ...
    // Use transaction for safe sale creation
    const result = await db.transaction(async (pool) => {
      // pool here is the wrapped transaction connection (client)
    ...
    const { saleId, saleNumber: savedSaleNumber, totalAmount: savedTotalAmount } = result;

    // Emit socket event
    const io = req.app.get('io');
    if (io) {
      io.to(`location-${locationId}`).emit('sale-completed', { saleId, saleNumber: savedSaleNumber, totalAmount: savedTotalAmount });
    }

    res.status(201).json({
      success: true,
      saleId,
      saleNumber: savedSaleNumber,
...
router.post('/park', async (req, res, next) => {
  try {
    ...
    res.json({ success: true, parkedId: result.insertId });
  } catch (error) {
    next(error);
  }
});
...
router.delete('/parked/:id', async (req, res, next) => {
  try {
    ...
    await db.query(`DELETE FROM parked_sales WHERE parked_id = $1`, [parseInt(id)]);
